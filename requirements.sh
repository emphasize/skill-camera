...
if os_is_like arch || os_is arch; then
    ...
    dependencies=( gst-plugins-base gst-plugins-base-libs gst-plugins-good gst-plugins-bad gst-plugins-ugly gst-plugin-opencv gst-libav )
...
if [[  $( uname -m ) == *"arm"* ]] && [[ -z $( os_is raspbian ) ]]; then
    if [[ -z $( pip freeze | grep opencv-python= ) ]] ; then
        ...
        if [[ $continue == 'y' ]] ; then
            ...
            echo
            echo "The wheels are stored in ~/opencv-python"
            echo 'install it with mycroft-pip install <wheel>'
        else
          echo "The opencv-python git was cloned to ~/opencv-python"
